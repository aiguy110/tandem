...
const PORT = Number(process.env.TANDEM_TEST_PORT || 7725);
...
async function main() {
  ...
  const upload = await fetch(`http://127.0.0.1:${PORT}/api/agents/${agentId}/assets`, {
    method: 'POST',
    headers: { authorization: `Bearer ${d1.token}`, 'content-type': 'image/png', 'x-file-name': 'restart.png' },
    body: PNG,
  });
  assetId = (await upload.json() as any).asset?.assetId ?? '';
  // The mock completes DERISK_IMAGE turns without an approval, leaving both the
  // user image reference and its ACP-observed response in the durable log.
  ws1.send(JSON.stringify({
    t: 'prompt', agentId,
    blocks: [{ type: 'text', text: 'DERISK_IMAGE restart' }, { type: 'image', assetId, mimeType: 'image/png' }],
  }));
  await sleep(900);
  ...
  const ws2 = await connect(d2.token, (f) => {
    ...
    if (f.t === 'snapshot') {
      ...
      f.transcript!.forEach((e) => {
        seenPost.push({ seq: e.seq, kind: e.event.kind });
        if (e.event.kind === 'user_message' && (e.event as any).blocks?.some((block: any) => block.assetId === assetId)) replayedImagePrompt = true;
      });
    }
  ...
  const restoredAssetResponse = await fetch(`http://127.0.0.1:${PORT}/api/agents/${agentId}/assets/${assetId}`, {
    headers: { authorization: `Bearer ${d2.token}` },
  });
  const restoredAsset = Buffer.from(await restoredAssetResponse.arrayBuffer());
  const assetRestored = restoredAssetResponse.status === 200 && restoredAsset.equals(PNG);
  ...
  const checks: [string, boolean, string][] = [
    ...
    ['image asset and transcript reference survived restart', assetRestored && replayedImagePrompt, `asset=${restoredAssetResponse.status}/${restoredAsset.length}B, prompt=${replayedImagePrompt}`],
  ];
