...
describe('audio engine position restore', () => {
  it('flushes position immediately on pause, on section change, and when the playlist ends', async () => {
    const sent: { sessionId: string; seq: number; positionMs: number }[] = [];
    setPositionSender((sessionId, seq, positionMs) => sent.push({ sessionId, seq, positionMs }));
    ...
    pause();
    expect(sent.at(-1)).toMatchObject({ sessionId: 'session-1', seq: 10 });
    const stored = JSON.parse(localStorage.getItem('tandem.audio.position.session-1')!);
    expect(stored).toMatchObject({ seq: 10 });

    play('session-1', 20); // section change: flushes the outgoing section (10)
  ...
  it('reconciliation: the daemon wins when its updatedAt is not older than the local copy', () => {
    setPlaylist('session-1', [10, 20]);
    localStorage.setItem('tandem.audio.position.session-1', JSON.stringify({ seq: 20, positionMs: 4000, updatedAt: 100 }));

    reconcileDaemonPosition('session-1', { seq: 10, positionMs: 1000, updatedAt: 200 });
  ...
  it('reconciliation: the local copy wins only when it is strictly newer than the daemon', () => {
    setPlaylist('session-2', [10, 20]);
    localStorage.setItem('tandem.audio.position.session-2', JSON.stringify({ seq: 20, positionMs: 4000, updatedAt: 300 }));

    reconcileDaemonPosition('session-2', { seq: 10, positionMs: 1000, updatedAt: 200 });
  ...
  it('restoring a saved position leaves the engine paused with three-state progress ready across the playlist', () => {
    setPlaylist('session-1', [10, 20, 30]);
    localStorage.setItem('tandem.audio.position.session-1', JSON.stringify({ seq: 20, positionMs: 5000, updatedAt: Date.now() }));

    restoreLocalPosition('session-1');
  ...
  it('falls back to no position when the saved seq no longer exists in the playlist', () => {
    setPlaylist('session-1', [10, 20]);
    localStorage.setItem('tandem.audio.position.session-1', JSON.stringify({ seq: 999, positionMs: 5000, updatedAt: Date.now() }));
  ...
  it('clamps a saved position that exceeds the clip\'s known duration', () => {
    setPlaylist('session-1', [10]);
    seedDurations('session-1', { 10: 8 });
    localStorage.setItem('tandem.audio.position.session-1', JSON.stringify({ seq: 10, positionMs: 20000, updatedAt: Date.now() }));
...
describe('getListeningAgentId (audio-focus retention predicate)', () => {
  it('returns the agent while a section is actually playing', async () => {
    ...
    expect(getListeningAgentId()).toBe('session-1');
  });
...
describe('silence keepalive', () => {
  ...
  it('does not write a position for the silence clip and preserves seq: 0 clear semantics', async () => {
    ...
    setPlaylist('session-1', []);
    Object.defineProperty(document, 'visibilityState', { configurable: true, get: () => 'hidden' });
    try {
      document.dispatchEvent(new Event('visibilitychange'));
      expect(sent.at(-1)).toMatchObject({ sessionId: 'session-1', seq: 0, positionMs: 0 });
    } finally {
  ...
  it('stops on teardown: the chat closing (playlist cleared) leaves nothing looping', async () => {
    ...
    setPlaylist('session-1', []); // same chat, e.g. the agent was closed
  ...
  it('lets a tap on the global player resume the real clip instead of leaving silence looping', async () => {
    ...
    play('session-1'); // "replay" tap on the global player
