...
- **Logging is currently insufficient for diagnosing production failures**, especially remote-agent/federation failures that leave sessions apparently idle or stuck. Progressively add structured logging at meaningful lifecycle, state-transition, network-boundary, queue, timeout, retry, and error paths whenever working nearby. Prefer enough context to reconstruct what happened (without logging secrets or full user prompts), and err toward too much operational logging rather than too little. The Go daemon uses the standard library's structured `log/slog`; continue using it consistently instead of ad-hoc prints.
