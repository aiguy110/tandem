...
func (r *Registry) SetAudioPosition(agentID string, seq, positionMs int64) (int64, error) {
	return r.store.SetAudioPosition(agentID, seq, positionMs)
}

// AudioPosition returns an agent's persisted audio player position, or nil
// if nothing is stored.
func (r *Registry) AudioPosition(agentID string) (*store.AudioPosition, error) {
	return r.store.AudioPosition(agentID)
}
