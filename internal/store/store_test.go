...
func TestFreshSchemaPragmasAndAgentLifecycle(t *testing.T) {
	...
	rows.Close()
	if want := []string{"agent_assets", "agents", "annotations", "assets", "browser_sessions", "browser_snapshots", "events", "history_entries", "history_entries_fts", "history_import_runs", "history_import_state", "history_sessions", "profile_recent", "profiles"}; !reflect.DeepEqual(tables, want) {
